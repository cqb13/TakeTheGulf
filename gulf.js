(() => {
  ...
  const findFunction = (stringRepresentation) => {
    return Object.keys(_).find(
      (key) =>
        _[key] && _[key].toString && _[key].toString() === stringRepresentation,
    );
  };
  ...
  /*
	 ...
	 */
  const jsonParsingFunctionName = findFunction(
    'function(a,b){const c=JSON.parse(a);if(Array.isArray(c))return new b(c);throw Error("U`"+a);}',
  );
  ...
  _[jsonParsingFunctionName] = function (a, b) {
    a = a
      ...
      .replaceAll("Gulf of America", "Gulf of Maksim");
  ...
  /*
	 ...
	 */
  const labelProcessingFunctionName = findFunction(
    "(a,b)=>{if(a.length!==0)return b(a[0])}",
  );
  ...
  const patchLabelBytesIfNeeded = (labelBytes) => {
    ...
    const SEARCH_PATTERN_BYTES = [..."Gulf of America"].map((char) =>
      char.charCodeAt(0),
    );
    ...
    const REPLACEMENT_BYTES = [..."Maksim\u200B"].map((char) =>
      char.charCodeAt(0),
    );

    // For every possible starting character in our `labelBytes` blob...
    for (
      let labelByteStartingIndex = 0;
      labelByteStartingIndex < labelBytes.length;
      labelByteStartingIndex++
    ) {
      ...
      for (
        let searchPatternIndex = 0;
        searchPatternIndex < SEARCH_PATTERN_BYTES.length;
        searchPatternIndex++
      ) {
        ...
        if (searchByte == CHAR_CODE_SPACE && !isAlphaChar(labelByte)) {
          ...
          do {
            ...
            !isAlphaChar(labelBytes[labelByteStartingIndex + labelByteOffset])
          );
      ...
      if (foundMatch) {
        ...
        const americaStartIndex = labelBytes.indexOf(
          CHAR_CODE_CAPITAL_A,
          labelByteStartingIndex,
        );
        ...
        for (let i = 0; i < labelBytes.length; i++) {
          if (
            labelBytes[i] == CHAR_CODE_PARENTH &&
            labelBytes[i + 1] == CHAR_CODE_CAPITAL_G
          ) {
            parenthStartIndex = i;
            break;
          }
